...
import { AlertTriangle, ShieldCheck, Activity, Filter } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ transactions, onSelectTx }) => {
  ...
  const pieData = [
    ...
    { name: 'Medium Risk', value: 0 }, // Simplified for demo
    { name: 'High Risk', value: transactions.filter(t => t.status === 'Flagged').length },
  ];

  // Get unique jurisdictions for filter dropdown
  const uniqueJurisdictions = useMemo(() => {
    return Array.from(new Set(transactions.map(t => t.jurisdiction))).sort();
  }, [transactions]);
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(tx => {
      ...
      const matchesJurisdiction = jurisdictionFilter === 'All' || tx.jurisdiction === jurisdictionFilter;
      return matchesStatus && matchesJurisdiction;
    });
  }, [transactions, statusFilter, jurisdictionFilter]);
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8">
      ...
      <div className="col-span-1 sm:col-span-2 lg:col-span-3 bg-secondary rounded-xl border border-slate-700 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-800/20">
            <div className="flex items-center justify-between sm:justify-start gap-4 w-full sm:w-auto">
              <h3 className="text-sm font-semibold text-slate-300 flex items-center gap-2">
                Recent Transactions
                <span className="text-xs font-normal text-slate-500 ml-1">({filteredTransactions.length})</span>
              </h3>
              <span className="text-[10px] md:text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded whitespace-nowrap border border-slate-700 sm:hidden">
                Vercel Live Feed
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
               <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <span className="hidden sm:inline-block text-[10px] md:text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded whitespace-nowrap border border-slate-700 ml-2">
                  Vercel Live Feed
                </span>
            </div>
        </div>
          ...
          <table className="w-full text-left text-sm text-slate-400">
            <thead className="bg-slate-800/50 text-xs uppercase font-medium text-slate-500">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold tracking-wider">Jurisdiction</th>
              ...
              {[...filteredTransactions].reverse().map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-800/40 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 ${['Seychelles', 'BVI', 'Cayman'].includes(tx.jurisdiction) ? 'text-red-400 font-semibold' : ''} text-xs md:text-sm`}>
                       {tx.jurisdiction}
                    </span>
                  </td>
