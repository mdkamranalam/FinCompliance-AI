...
const FALLBACK_LIVE_TRANSACTIONS: Omit<Transaction, 'id' | 'status' | 'timestamp'>[] = [
  {
    amount: 875000,
    currency: "INR",
...
export const generateSTRAnalysis = async (tx: Transaction, velocityCount: number = 1): Promise<{ narrative: string; xml: string }> => {
  ...
  const prompt = `
    ...
    - Jurisdiction Risk: Transfers to tax havens or high-risk countries (e.g., ${tx.receiver_country}) require enhanced due diligence.
  ...
  try {
    ...
    console.warn("Gemini API Error (likely quota). Using fallback narrative.", error);
    return {
...
export const fetchRealTimeTransactions = async (): Promise<Omit<Transaction, 'id' | 'status' | 'timestamp'>[]> => {
  const model = "gemini-2.5-flash";
  const prompt = `
  ...
  try {
    ...
    console.warn("Gemini API Error fetching transactions (likely quota). Using Mock Data.", error);
